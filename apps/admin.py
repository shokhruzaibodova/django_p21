...
from apps.models import Product, Category, StudentUser, TeacherUser, SubjectUser
...
@admin.register(SubjectUser)
class SubjectsUserModelAdmin(UserAdmin):
    def get_queryset(self, request):
        return super().get_queryset(request).filter(type=StudentUser.Type.SUBJECT)
